...
func Fetch() error {
	defer log.Printf("[%s] Fetch Done.", configs.Data.MS["kyodonews"].Title)
	log.Printf("[%s] Fetching ...", configs.Data.MS["kyodonews"].Title)
			...
			if err != nil {
				if !errors.Is(err, ErrTimeOverDays) {
					log.Printf("[%s] fetch error: %v, link: %s",
						configs.Data.MS["kyodonews"].Title, err, link)
				}
				err = nil
				continue
			}
