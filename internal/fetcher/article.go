...
import (
	...
	"github.com/hi20160616/ms-kyodonews/configs"
...
func NewArticle() *Article {
	return &Article{
		WebsiteDomain: configs.Data.MS["kyodonews"].Domain,
		WebsiteTitle:  configs.Data.MS["kyodonews"].Title,
		WebsiteId:     fmt.Sprintf("%x", md5.Sum([]byte(configs.Data.MS["kyodonews"].Domain))),
	}
}
...
func (a *Article) Get(id string) (*Article, error) {
	...
	return nil, fmt.Errorf("[%s] no article with id: %s",
		configs.Data.MS["kyodonews"].Title, id)
}
...
var timeout = func() time.Duration {
	t, err := time.ParseDuration(configs.Data.MS["kyodonews"].Timeout)
	if err != nil {
		log.Printf("[%s] timeout init error: %v", configs.Data.MS["kyodonews"].Title, err)
...
func (a *Article) fetchTitle() (string, error) {
	...
	for _, v := range n {
		if v.FirstChild != nil {
			return gears.ChangeIllegalChar(v.FirstChild.Data), nil
		}
	}
	return "", fmt.Errorf("none title: %s", a.U.String())
}
...
func (a *Article) fetchContent() (string, error) {
	if a.doc == nil {
		return "", errors.Errorf("[%s] fetchContent: doc is nil: %s", configs.Data.MS["kyodonews"].Title, a.U.String())
	}
	body := ""
